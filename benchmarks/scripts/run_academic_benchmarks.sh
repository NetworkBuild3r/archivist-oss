...
if [ -f "$LONGMEM_FILE" ]; then
  python -m benchmarks.academic.longmemeval.adapter $LONGMEM_ARGS || {
    echo "  ⚠ LongMemEval failed (continuing)" >&2
    if ! grep -q '"traceback"' "$LONGMEM_OUT" 2>/dev/null; then
      echo '{"error":"LongMemEval adapter exited non-zero","source":"run_academic_benchmarks.sh","hint":"See stderr; use benchmarks/scripts/run_longmemeval_docker.sh for timestamped JSON + compose networking."}' > "$LONGMEM_OUT"
    fi
  }
else
...
if [ -d "$LOCOMO_DIR" ] && [ -n "$(ls -A "$LOCOMO_DIR" 2>/dev/null)" ]; then
  python -m benchmarks.academic.locomo.adapter $LOCOMO_ARGS || {
    echo "  ⚠ LoCoMo failed (continuing)" >&2
    if ! grep -q '"traceback"' "$LOCOMO_OUT" 2>/dev/null; then
      echo '{"error":"LoCoMo adapter exited non-zero","source":"run_academic_benchmarks.sh"}' > "$LOCOMO_OUT"
    fi
  }
else
...
if [ -d "$HALUMEM_DIR" ] && [ -n "$(ls -A "$HALUMEM_DIR" 2>/dev/null)" ]; then
  python -m benchmarks.academic.halumem.adapter $HALUMEM_ARGS || {
    echo "  ⚠ HaluMem failed (continuing)" >&2
    if ! grep -q '"traceback"' "$HALUMEM_OUT" 2>/dev/null; then
      echo '{"error":"HaluMem adapter exited non-zero","source":"run_academic_benchmarks.sh"}' > "$HALUMEM_OUT"
    fi
  }
else
