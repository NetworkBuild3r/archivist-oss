#!/usr/bin/env bash
# LongMemEval ablation (all 4 pipeline variants) inside Docker.
# Timestamped output under .benchmarks/runs/.
#
# Usage (from repo root):
#   bash benchmarks/scripts/run_ablation_docker.sh
#   bash benchmarks/scripts/run_ablation_docker.sh -d                     # detached
#   bash benchmarks/scripts/run_ablation_docker.sh --limit 50 --no-refine
#   bash benchmarks/scripts/run_ablation_docker.sh --variants vector_only,full_pipeline
#
set -euo pipefail
...
OUT="${RUN_DIR}/ablation_${STAMP}_${GIT_SHA}.json"
LATEST=".benchmarks/ablation_latest.json"
CID_FILE=".benchmarks/ablation.cid"

echo "================================================"
echo "  LongMemEval Ablation (Docker)"
echo "  Output:   $OUT"
echo "  Latest:   $LATEST"
