...
class TestBM25Modes:
    def test_fts5_and_query_removes_stopwords(self):
        from archivist.storage.fts_search import _fts5_and_query
    ...
    def test_fts5_phrase_query(self):
        from archivist.storage.fts_search import _fts5_phrase_query
    ...
    def test_fts5_safe_query_or_mode(self):
        from archivist.storage.fts_search import _fts5_safe_query
...
class TestDynamicThreshold:
    def test_keeps_minimum_results(self):
        from archivist.retrieval.retrieval_filters import apply_dynamic_threshold
...
class TestEmbeddingCache:
    def test_cache_hit_returns_same_vector(self):
        from archivist.features.embeddings import _cache_get, _cache_put
    ...
    def test_cache_miss_returns_none(self):
        from archivist.features.embeddings import _cache_get

        assert _cache_get("never_stored_xyz_12345", "test-model") is None

    def test_cache_ttl_expiry(self):
        from archivist.features.embeddings import (
            _cache_get,
            _cache_key,
            _cache_put,
            _embed_cache,
...
class TestQueryExpansionCache:
    def test_expansion_cache_roundtrip(self):
        from archivist.retrieval.query_expansion import _cache_get, _cache_put
    ...
    def test_expansion_cache_miss(self):
        from archivist.retrieval.query_expansion import _cache_get

        assert _cache_get("never_asked_query_xyz_99999") is None

    def test_expansion_cache_ttl_expiry(self):
        from archivist.retrieval.query_expansion import (
            _cache_get,
            _cache_put,
            _expansion_cache,
            _expansion_lock,
...
def _generate_questions_or_skip():
    ...
    if not os.path.isfile(corpus_py):
        pytest.skip(
            "benchmarks/fixtures/generate_corpus.py not present (benchmarks/ often gitignored)"
        )
