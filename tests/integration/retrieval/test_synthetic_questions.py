...
class TestSyntheticQuestionParsing:
    def test_parse_json_array(self):
        from archivist.write.synthetic_questions import _parse_questions
...
class TestSyntheticQuestionGeneration:
    ...
    async def test_generates_questions_from_llm(self, monkeypatch):
        ...
        mock_response = (
            '["What is the backup schedule?", "When do backups run?", "How is data backed up?"]'
        )
        with patch(
            ...
            result = await synthetic_questions.generate_synthetic_questions(
