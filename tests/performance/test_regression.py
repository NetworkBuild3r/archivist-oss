...
Run subset::
    ...
    pytest tests/performance/test_regression.py -v --tb=short -s
"""
...
async def test_schema_ddl_under_200ms(tmp_path, monkeypatch):
    ...
    from tests.fixtures.schema import build_schema as _build_schema

    async def _build_schema_wrapper(pool):
        """Wrap build_schema to match old signature (takes pool, not conn)."""
        async with pool.write() as conn:
            await _build_schema(conn)

    t0 = time.perf_counter()
    await _build_schema_wrapper(p)
    elapsed = _ms(t0)
